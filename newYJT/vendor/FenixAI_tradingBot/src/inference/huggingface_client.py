        ...
        for attempt in range(max_attempts):
            ...
            try:
                ...
                try:
                    ...
                    latency_ms = (time.time() - start_time) * 1000
                    self._update_stats(latency_ms, success=True)
                    logger.info(f"✅ HF text_generation fallback: {model_id} → {len(text)} chars en {latency_ms:.0f}ms")
                    return text
                except Exception:
                ...
                if '429' in error_msg or 'too many requests' in error_msg.lower() or 'rate limit' in error_msg.lower():
                    ...
                    logger.warning(f"🟡 Rate limit HF para {model_id}. Intento {attempt+1}/{max_attempts}, esperando {backoff_s}s")
                    self.stats['rate_limited'] += 1
    ...
    async def generate_with_vision(
        ...
        for attempt in range(max_attempts):
            ...
            try:
                ...
                logger.info(f"✅ HF API v2 (Vision): {model_id} → {len(text)} chars en {latency_ms:.0f}ms")
                return text
                ...
                if '429' in error_msg or 'too many requests' in error_msg.lower() or 'rate limit' in error_msg.lower():
                    ...
                    logger.warning(f"🟡 Rate limit HF (Vision) para {model_id}. Intento {attempt+1}/{max_attempts}, esperando {backoff_s}s")
                    self.stats['rate_limited'] += 1
                    await asyncio.sleep(backoff_s)
                    if attempt < max_attempts - 1:
                        continue
                ...
                if 'not supported' in error_msg.lower() or 'unsupported' in error_msg.lower():
                    logger.error(f"❌ Modelo {model_id} no soporta entrada de visión vía chat.completions")
                    self._update_stats(latency_ms, success=False)
