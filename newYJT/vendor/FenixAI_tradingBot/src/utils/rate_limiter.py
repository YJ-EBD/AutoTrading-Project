...
class ModelRateLimiter:
    ...
    async def acquire(self, priority: int = 1) -> bool:
        ...
        try:
            ...
            if wait_time > 30:  # Don't wait more than 30 seconds
                logger.warning(f"🟡 [{self.model_id}] Tiempo de espera muy largo ({wait_time:.1f}s), rechazando")
                return False
    ...
    def record_failure(self, is_rate_limit: bool = False) -> None:
        ...
        if is_rate_limit:
            ...
            logger.warning(f"🟡 [{self.model_id}] Rate limited, iniciando backoff: {backoff_duration:.1f}s")
