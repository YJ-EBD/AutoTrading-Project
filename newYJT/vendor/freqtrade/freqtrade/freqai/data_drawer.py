...
class FreqaiDataDrawer:
    ...
    def load_global_metadata_from_disk(self):
        ...
        if exists:
            try:
                with self.global_metadata_path.open("r", encoding="utf-8", errors="replace") as fp:
                    metatada_dict = rapidjson.load(fp, number_mode=rapidjson.NM_NATIVE)
                    return metatada_dict
            except (rapidjson.JSONDecodeError, OSError):
                logger.warning("Global metadata file is empty or corrupted. Starting with empty metadata.")
        return {}
    ...
    def load_drawer_from_disk(self):
        ...
        if exists:
            try:
                with self.pair_dictionary_path.open("r", encoding="utf-8", errors="replace") as fp:
                    self.pair_dict = rapidjson.load(fp, number_mode=rapidjson.NM_NATIVE)
            except (rapidjson.JSONDecodeError, OSError):
                logger.warning(
                    "Data drawer pair dictionary is empty or corrupted at %s. Starting from scratch.",
                    self.pair_dictionary_path,
                )
                self.pair_dict = {}
        else:
        ...
        if exists:
            try:
                ...
                try:
                    ...
                    logger.warning(
                        "Backup historical prediction file is also corrupted. Starting without historic predictions."
                    )
    ...
    def set_initial_return_values(
        ...
        new_pred["date_pred"] = pd.to_datetime(dataframe["date"], utc=True)
    ...
    def append_model_predictions(
        ...
        label_stats_mean = dk.data.get("labels_mean", {})
        label_stats_std = dk.data.get("labels_std", {})

        # model outputs and associated statistics
        for label in predictions.columns:
            resolved_label = label
            if resolved_label not in df.columns and str(resolved_label) in df.columns:
                resolved_label = str(resolved_label)
            if resolved_label not in df.columns:
                logger.debug(
                    "Skipping unresolved prediction column '%s' for pair %s. Known columns: %s",
                    label,
                    pair,
                    list(df.columns),
                )
            ...
            label_loc = df.columns.get_loc(resolved_label)
            pred_label_loc = predictions.columns.get_loc(label)
            df.iloc[-1, label_loc] = predictions.iloc[-1, pred_label_loc]
            if df[resolved_label].dtype == object:
                continue

            stats_label = resolved_label
            if stats_label not in label_stats_mean and str(stats_label) in label_stats_mean:
                stats_label = str(stats_label)

            mean_column = f"{resolved_label}_mean"
            std_column = f"{resolved_label}_std"
            if (
                mean_column in df.columns
                and std_column in df.columns
                and stats_label in label_stats_mean
                and stats_label in label_stats_std
            ):
                label_mean_loc = df.columns.get_loc(mean_column)
                label_std_loc = df.columns.get_loc(std_column)
                df.iloc[-1, label_mean_loc] = label_stats_mean[stats_label]
                df.iloc[-1, label_std_loc] = label_stats_std[stats_label]
        ...
        df.iloc[-1, date_pred_loc] = pd.to_datetime(strat_df.iloc[-1, date_loc], utc=True)
        ...
        with self.history_lock:
            ...
            for pair in dk.all_pairs:
                for tf in feat_params.get("include_timeframes"):
                    hist_df = history_data[pair][tf].copy()
                    hist_df["date"] = pd.to_datetime(hist_df["date"], utc=True)
                    history_data[pair][tf] = hist_df
                    # check if newest candle is already appended
                    df_dp = strategy.dp.get_pair_dataframe(pair, tf).copy()
                    if len(df_dp.index) == 0:
                        continue
                    df_dp["date"] = pd.to_datetime(df_dp["date"], utc=True)
                    hist_last_date = hist_df.iloc[-1]["date"]
                    dp_first_date = df_dp["date"].iloc[0]
                    dp_last_date = df_dp["date"].iloc[-1]
                    if hist_last_date == dp_last_date:
                        continue

                    try:
                        index = df_dp.loc[df_dp["date"] == hist_last_date].index[0] + 1
                    except IndexError:
                        if hist_last_date < dp_first_date:
                            raise OperationalException(
                            ...
                            logger.warning(
                                ...
                                "data. \n"
                                f"Historical data ends at {hist_last_date} "
                                f"while dataprovider starts at {dp_first_date} and"
                                f"ends at {dp_last_date}."
                            )
            ...
            self.current_candle = pd.to_datetime(
                history_data[dk.pair][self.config["timeframe"]].iloc[-1]["date"],
                utc=True,
            )
